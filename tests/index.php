...
/*
 * PHP-Auth (https://github.com/ExecPHP-im/PHP-Auth)
 * Copyright (c) ExecPHP.im (https://www.ExecPHP.im/)
...
function processRequestData(\ExecPHP\Auth $auth) {
	if (isset($_POST)) {
		if (isset($_POST['action'])) {
			if ($_POST['action'] === 'login') {
				...
				catch (\ExecPHP\InvalidEmailException $e) {
					return 'wrong email address';
				}
				catch (\ExecPHP\UnknownUsernameException $e) {
					return 'unknown username';
				}
				catch (\ExecPHP\AmbiguousUsernameException $e) {
					return 'ambiguous username';
				}
				catch (\ExecPHP\InvalidPasswordException $e) {
					return 'wrong password';
				}
				catch (\ExecPHP\EmailNotVerifiedException $e) {
					return 'email address not verified';
				}
				catch (\ExecPHP\TooManyRequestsException $e) {
				...
				catch (\ExecPHP\UserAlreadyExistsException $e) {
					return 'email address already exists';
				}
				catch (\ExecPHP\DuplicateUsernameException $e) {
				...
				catch (\ExecPHP\InvalidSelectorTokenPairException $e) {
					return 'invalid token';
				}
				catch (\ExecPHP\TokenExpiredException $e) {
				...
				catch (\ExecPHP\ConfirmationRequestNotFound $e) {
				...
				catch (\ExecPHP\ResetDisabledException $e) {
				...
				catch (\ExecPHP\NotLoggedInException $e) {
				...
				if (isset($_POST['id'])) {
					...
					catch (\ExecPHP\UnknownIdException $e) {
						return 'unknown ID';
					}
				}
...
function showDebugData(\ExecPHP\Auth\Auth $auth, $result) {
	echo '<pre>';
	...
	\var_dump($auth->hasRole(\ExecPHP\Auth\Role::SUPER_MODERATOR));

	echo 'Roles (developer *or* manager)' . "\t\t";
	\var_dump($auth->hasAnyRole(\ExecPHP\Auth\Role::DEVELOPER, \ExecPHP\Auth\Role::MANAGER));

	echo 'Roles (developer *and* manager)' . "\t\t";
	\var_dump($auth->hasAllRoles(\ExecPHP\Auth\Role::DEVELOPER, \ExecPHP\Auth\Role::MANAGER));
	...
	\var_dump(\ExecPHP\Auth\Auth::createRememberCookieName());
	echo "\n";

	echo 'Auth::createCookieName(\'session\')' . "\t";
	\var_dump(\ExecPHP\Auth\Auth::createCookieName('session'));
	echo 'Auth::createRandomString()' . "\t\t";
	\var_dump(\ExecPHP\Auth\Auth::createRandomString());
	echo 'Auth::createUuid()' . "\t\t\t";
	\var_dump(\ExecPHP\Auth\Auth::createUuid());

	echo '</pre>';
}

function convertStatusToText(\ExecPHP\Auth\Auth $auth) {
	if ($auth->isLoggedIn() === true) {
		if ($auth->getStatus() === \ExecPHP\Auth\Status::NORMAL && $auth->isNormal()) {
			return 'normal';
		}
		elseif ($auth->getStatus() === \ExecPHP\Auth\Status::ARCHIVED && $auth->isArchived()) {
			return 'archived';
		}
		elseif ($auth->getStatus() === \ExecPHP\Auth\Status::BANNED && $auth->isBanned()) {
			return 'banned';
		}
		elseif ($auth->getStatus() === \ExecPHP\Auth\Status::LOCKED && $auth->isLocked()) {
			return 'locked';
		}
		elseif ($auth->getStatus() === \ExecPHP\Auth\Status::PENDING_REVIEW && $auth->isPendingReview()) {
			return 'pending review';
		}
		elseif ($auth->getStatus() === \ExecPHP\Auth\Status::SUSPENDED && $auth->isSuspended()) {
			return 'suspended';
		}
	}
...
function showAuthenticatedUserForm(\ExecPHP\Auth\Auth $auth) {
...
function createRolesOptions() {
	...
	foreach (\ExecPHP\Auth\Role::getMap() as $roleValue => $roleName) {

...
final class Auth extends UserManager {
	...
	const COOKIE_PREFIXES = [ self::PREFIX_SECURE, self::PREFIX_HOST ];
	...
	private function enhanceHttpSecurity() {
		...
		if ($this->isLoggedIn()) {
			...
			$this->response->withHeader('Expires','Thu, 19 Nov 1981 00:00:00 GMT');
	...
	public function isLoggedIn() {
		return $this->session->get(self::SESSION_FIELD_LOGGED_IN, false);
	}
	...
	public function getUserId() {
		return $this->session->get(self::SESSION_FIELD_USER_ID, null);
	}
	...
	public function getEmail() {
		return $this->session->get(self::SESSION_FIELD_EMAIL, null);
	...
	public function getUsername() {
		return $this->session->get(self::SESSION_FIELD_USERNAME, null);
	}
	...
	public function getStatus() {
		return $this->session->get(self::SESSION_FIELD_STATUS, null);
	}
	...
	public function hasRole($role) {
		...
		$rol = $this->session->get(self::SESSION_FIELD_ROLES, false);

		if ($rol === false)
			return false;
		else{
			return ((int) $rol & $role) === (int) $role;
		}
	}
	...
	public function isRemembered() {
		return $this->session->get(self::SESSION_FIELD_REMEMBERED, null);
	}

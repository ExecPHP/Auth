...
catch (\ExecPHP\Auth\InvalidEmailException $e) {
    die('Invalid email address');
}
catch (\ExecPHP\Auth\InvalidPasswordException $e) {
    die('Invalid password');
}
catch (\ExecPHP\Auth\UserAlreadyExistsException $e) {
    die('User already exists');
}
catch (\ExecPHP\Auth\TooManyRequestsException $e) {
...
catch (\ExecPHP\Auth\EmailNotVerifiedException $e) {
...
catch (\ExecPHP\Auth\InvalidSelectorTokenPairException $e) {
    die('Invalid token');
}
catch (\ExecPHP\Auth\TokenExpiredException $e) {
    die('Token expired');
}
catch (\ExecPHP\Auth\UserAlreadyExistsException $e) {
...
catch (\ExecPHP\Auth\ResetDisabledException $e) {
...
catch (\ExecPHP\Auth\NotLoggedInException $e) {
    die('Not logged in');
}
catch (\ExecPHP\Auth\InvalidPasswordException $e) {
...
catch (\ExecPHP\Auth\ConfirmationRequestNotFound $e) {
...
Here’s how to use this library with your own tables for custom user information in a maintainable and re-usable way:
 ...
 1. If you need the custom user information only rarely, you may just retrieve it as needed. If you need it more frequently, however, you’d probably want to have it in your session data. The following method is how you can load and access your data in a reliable way:
    ...
    ```php
    function getUserInfo(\ExecPHP\Auth\Auth $auth) {
        if (!$auth->isLoggedIn()) {
            return null;
        }
...
```php
if ($auth->hasRole(\ExecPHP\Auth\Role::SUPER_MODERATOR)) {
    echo 'The user is a super moderator';
}

// or

if ($auth->hasAnyRole(\ExecPHP\Auth\Role::DEVELOPER, \ExecPHP\Auth\Role::MANAGER)) {
    echo 'The user is either a developer, or a manager, or both';
}

// or

if ($auth->hasAllRoles(\ExecPHP\Auth\Role::DEVELOPER, \ExecPHP\Auth\Role::MANAGER)) {
...
```php
\ExecPHP\Auth\Role::ADMIN;
\ExecPHP\Auth\Role::AUTHOR;
\ExecPHP\Auth\Role::COLLABORATOR;
\ExecPHP\Auth\Role::CONSULTANT;
\ExecPHP\Auth\Role::CONSUMER;
\ExecPHP\Auth\Role::CONTRIBUTOR;
\ExecPHP\Auth\Role::COORDINATOR;
\ExecPHP\Auth\Role::CREATOR;
\ExecPHP\Auth\Role::DEVELOPER;
\ExecPHP\Auth\Role::DIRECTOR;
\ExecPHP\Auth\Role::EDITOR;
\ExecPHP\Auth\Role::EMPLOYEE;
\ExecPHP\Auth\Role::MAINTAINER;
\ExecPHP\Auth\Role::MANAGER;
\ExecPHP\Auth\Role::MODERATOR;
\ExecPHP\Auth\Role::PUBLISHER;
\ExecPHP\Auth\Role::REVIEWER;
\ExecPHP\Auth\Role::SUBSCRIBER;
\ExecPHP\Auth\Role::SUPER_ADMIN;
\ExecPHP\Auth\Role::SUPER_EDITOR;
\ExecPHP\Auth\Role::SUPER_MODERATOR;
\ExecPHP\Auth\Role::TRANSLATOR;
```
...
```php
\ExecPHP\Auth\Role::getMap();
// or
\ExecPHP\Auth\Role::getNames();
// or
\ExecPHP\Auth\Role::getValues();
```
...
```php
function canEditArticle(\ExecPHP\Auth\Auth $auth) {
...
final class MyRole {
    ...
    const CUSTOMER_SERVICE_AGENT = \ExecPHP\Auth\Role::REVIEWER;
    const FINANCIAL_DIRECTOR = \ExecPHP\Auth\Role::COORDINATOR;
...
catch (\ExecPHP\Auth\UnknownIdException $e) {
    die('Unknown ID');
}
```
...
catch (\ExecPHP\Auth\UnknownUsernameException $e) {
    die('Unknown username');
}
catch (\ExecPHP\Auth\AmbiguousUsernameException $e) {
...
```php
try {
    $auth->admin()->addRoleForUserById($userId, \ExecPHP\Auth\Role::ADMIN);
}
catch (\ExecPHP\Auth\UnknownIdException $e) {
    die('Unknown user ID');
}
...
try {
    $auth->admin()->addRoleForUserByEmail($userEmail, \ExecPHP\Auth\Role::ADMIN);
}
catch (\ExecPHP\Auth\InvalidEmailException $e) {
    die('Unknown email address');
}
...
try {
    $auth->admin()->addRoleForUserByUsername($username, \ExecPHP\Auth\Role::ADMIN);
}
catch (\ExecPHP\Auth\UnknownUsernameException $e) {
    die('Unknown username');
}
...
```php
try {
    $auth->admin()->removeRoleForUserById($userId, \ExecPHP\Auth\Role::ADMIN);
}
catch (\ExecPHP\Auth\UnknownIdException $e) {
    die('Unknown user ID');
}
...
try {
    $auth->admin()->removeRoleForUserByEmail($userEmail, \ExecPHP\Auth\Role::ADMIN);
}
catch (\ExecPHP\Auth\InvalidEmailException $e) {
    die('Unknown email address');
}
...
try {
    $auth->admin()->removeRoleForUserByUsername($username, \ExecPHP\Auth\Role::ADMIN);
}
catch (\ExecPHP\Auth\UnknownUsernameException $e) {
    die('Unknown username');
}
...
```php
try {
    if ($auth->admin()->doesUserHaveRole($userId, \ExecPHP\Auth\Role::ADMIN)) {
...
```php
\ExecPHP\Auth\Auth::createRememberCookieName();
```
...
```php
$length = 24;
$randomStr = \ExecPHP\Auth\Auth::createRandomString($length);
```

#### Creating a UUID v4 as per RFC 4122

```php
$uuid = \ExecPHP\Auth\Auth::createUuid();
```
